...
mode_selection_threshold = 0.45;   % crown改成了以前后帧的速度转角为判断标准
                                    % 最开始认为如果左右轮轮速差小于mode_selection_threshold，认为直行。（阈值0.0000000001）
for i=1:1:len-2
    T=t(i+1)-t(i);
...
%     Cnb_n = angle2dcm(deg2rad(HPR(i+1,3)),deg2rad(HPR(i+1,2)),deg2rad(HPR(i+1,1)+90),'YXZ');
    ...
    if rad2deg(theta)<mode_selection_threshold   %如果直行
        ...
        if (L(i)-R(i))>0
            ...
            rot_Z_axes_in_N = rot_Z_axes_in_N/norm(rot_Z_axes_in_N);    % 旋转系的Y轴在N系下的表示，要单位化
            rot_Y_axes_in_N = cross(rot_Z_axes_in_N, rot_X_axes_in_N);   
...
axis equal;
grid on;xlabel('x');ylabel('y');zlabel('z');title('3D解算结果');

figure;
position_3d_to_2d = position_3d(:,1:2);
position_3d_to_2d_x0 = position_3d_to_2d(:,1) * 1;
position_3d_to_2d_y0 = position_3d_to_2d(:,2) * 1;
xuanzhuan = -90*3.1415925/180;
position_3d_to_2d_x = position_3d_to_2d_x0*cos(xuanzhuan)+position_3d_to_2d_y0*sin(xuanzhuan);
position_3d_to_2d_y = position_3d_to_2d_y0*cos(xuanzhuan)-position_3d_to_2d_x0*sin(xuanzhuan);
plot(position_x,position_y,'r.' , 'LineWidth',1.5);
hold on;
plot(position_3d_to_2d_x,position_3d_to_2d_y,'.','linewidth',1.5);
plot(position_3d_to_2d_x(1),position_3d_to_2d_y(1),'o','linewidth',1.5);
legend('原程序2D解算结果','3D解算的XY投影');xlabel('x');ylabel('y');title('3D解算向XY平面的投影');
