...
%2、高度限制
%3、角度限制
%%原理参见 Vehicle Constraints Enhancement for Supporting INS Navigation in Urban Environments Itzik Klein
%这里的限制都是比较简单的
glvs%这里初始化了一个全局变量，应该是用来进行参数传递的
psinstypedef(153);%决定状态参数的数量，这里就是15个的意思
trj = trjfile('trj11ms.mat');
% initial settings
...
for k=1:nn:len-nn+1
    ...
    %%
    %%%速度限制部分，原理是认为除了前进方向外其他方向加速度已知
    %wvm(:,4)=0;
    %%
    %%高度限制部分，就是认为高度方向减去重力加速度为零
    %wvm(:,6)=9.80*ts;gravity增量
    %%
    %%角度限制部分，就是认为车辆只有航向角发生变化，其他角度不发生变化
    %wvm(:,1:2)=0;
    %以上限制不能单独使用，可能加入到IMM的限制中去就能够使用了
